...
int main(int argc, char **argv)
{
    ...
    if (argc==1) {
    	...
    	printf("\t-var <int>\n"); //prnnlm
        printf("\t\tSets size of the latent variable; default is 0\n"); //prnnlm
    ...
    i=argPos((char *)"-var", argc, argv);
    if (i>0) {
        if (i+1==argc) {
            printf("ERROR: variable size not specified!\n");
            return 0;
        }

        variable_size=atoi(argv[i+1]);

        if (debug_mode>0)
        printf("latent variable size: %d\n", variable_size);
    }
    ...
    if (train_mode) {
        ...
        //prnnlm
        model1.setVariableSize(variable_size);
        //end prnnlm	
